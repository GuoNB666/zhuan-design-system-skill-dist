...
- **Runtime conflict**: the generated artifact cannot directly render `IOSDevice`, `TopNav`, `Icon`, `ChipRail`/`Chip`, `ProductCardH`, `Sheet`, and `Button` from `ui_kits/zhuan-app`.
...
3. Use real component calls.
   ...
   - The output must contain real calls for relevant existing components, such as `<IOSDevice`, `<TopNav`, `<Icon`, `<SearchHeader`, `<SortTabs`, `<ChipRail`, `<Chip`, `<ProductCardH`, `<ProductCardFeeds`, `<Sheet`, `<Toast`, and `<Button`.
   - The artifact must fail self-review if these strings are absent from the generated source for an App/H5 flow: `<IOSDevice`, `<TopNav`, and at least three relevant DS components such as `<Button`, `<ButtonBar`, `<Sheet`, `<Toast`, `<Chip`, `<ProductCardH`, `<OrderCard`, `<FormGroup`, or `<Icon`.
   - App/H5/mobile artifacts must wrap the delivered screen or flow in `<IOSDevice>`. Do not create a custom `.phone`, `.status-spacer`, or fake iOS frame.
   - Use `SearchHeader` and `ChipRail` for list/search/filter rails when the screen has those patterns.
   - Use `FormGroup`, `FormInput`, `FormRadioRow`, `FormVerticalRadioList`, and `ButtonBar` for form/action surfaces when available.
   - Use `OrderCard` for order-list, fulfillment-list, and after-sales-list cards.
   - Use `OrderProductCard` from `OrderDetailCards.jsx` for order detail, refund detail, after-sales detail, after-sales handling summaries, return-to-user summaries, and product-summary sections. Do not use `OrderCard` merely because the copy contains "订单", "售后", or "商品"; first classify the surface as list card versus detail summary.
   - Use `Toast` for success, error, and confirmation feedback. Never create `.toast` or an ad hoc floating feedback layer. Default toast placement is the device center; only use `placement="bottom"` or `bottom` when the brief explicitly needs to avoid a keyboard/bottom bar.
   ...
   - Brand assets and icons must resolve through DS components/resolvers such as `Icon` and `OrderTrustBadge`. Do not hardcode relative `.assets/...` paths in page code; OpenDesign file preview and raw preview can serve hidden assets from different routes.
...
- Real calls: The artifact contains actual component calls for the selected surface, not only CSS class names or `data-component` markers.
- Source-string gate: App/H5 source contains `<IOSDevice`, `<TopNav`, and at least three additional relevant DS component calls. If not, fail the output.
- Shell lock: App/H5/mobile output uses `<IOSDevice>`; no custom `.phone`, `.status-spacer`, or fake iOS frame.
- Pattern lock: search/filter rails use `SearchHeader` / `ChipRail` when present; form pages use `FormGroup` / `Form*` / `ButtonBar` when present.
- Order/detail selection lock: order/after-sales lists use `OrderCard`; detail/product-summary sections use `OrderProductCard` / `OrderDetailCards` when available. Flag `OrderCard` inside a detail summary as a misuse unless it is intentionally presenting a full list-style order card.
...
```text
使用转转设计系统 v0.2.9，并启用 zhuan-design-system-usage-review。
...
App/H5 页面必须用 IOSDevice 包裹主流程，并真实调用 TopNav、Icon、Chip/ChipRail、ProductCardH、Sheet、Toast、Button。
源码中必须能直接搜索到 <IOSDevice、<TopNav，以及至少 3 个相关 DS 组件调用；否则视为生成失败。
列表/筛选优先调用 SearchHeader、ChipRail；表单/底部操作优先调用 FormGroup、FormInput、FormRadioRow、FormVerticalRadioList、ButtonBar。
订单/售后列表卡优先调用 OrderCard；订单详情、退款详情、售后详情、售后处理摘要、寄回用户商品摘要优先调用 OrderProductCard / OrderDetailCards，不得因为名称相近就把列表型 OrderCard 硬塞为详情摘要卡。
...
组件调用检查：通过；使用 IOSDevice 和真实组件调用，未用 HTML/CSS 仿制。
